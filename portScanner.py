...
allModules = [["http", "Scan for open HTTP ports, and get the titles."],
              ...
              ["ssh", "Scan for open SSH ports."], ["printer",
                                                    "Scan for open printer ports and websites."],
...
textToModule = [["http", http], ["template", template], ["printer", printer], [
    "gameserver", gameserver], ["ssh", ssh], ["manual", manual], ["mongodb", mongodb], ["mysql", mysql]]
...
def commandHandler(command):
    ...
    def helpPrint(name, desc, usage):
        print("\t" + YELLOW + name + GREEN + ": " + BLUE +
              desc + GREEN + " - '" + usage + "'" + END)
    if command == "help":
    ...
    # USE
        ...
        if not inModule:
            ...
            if inModule:
                ...
                getCoreOptions = getattr(
                    currentModuleFile, "coreOptions", None)
                moduleOptions = getCoreOptions()
            else:
                print(RED + "[!] Module '" + YELLOW +
                      tempModule + RED + "' not found." + END)
        else:
            print(RED + "[!] Module '" + YELLOW + currentModule + RED + "' already selected. Type '" +
                  YELLOW + "back" + RED + "' to go back to the main menu." + END)
    elif command == "use":
        print(RED + "[!] Usage: 'use " + YELLOW +
              "module_name" + RED + "'" + END)
    ...
    # OPTIONS
        ...
        if inModule:
            print(GREEN + "\n Options for module '" +
                  YELLOW + currentModule + GREEN + "':" + END)
            for option in moduleOptions:
                if option[2] == "":
                    print("\t" + YELLOW + option[0] + GREEN + " - " + BLUE +
                          option[1] + GREEN + " ==> " + RED + "[NOT SET]" + END)
                else:
    ...
    # SET
        ...
        if inModule:
            ...
            try:
                ...
                print(RED + "[!] Usage: 'set " + YELLOW +
                      "option_name option_value" + RED + "'" + END)
                error = True
            if not error:
                inOptions = False
                for option in moduleOptions:
                    if option[0] == command[0]:
                        inOptions = True
                        option[2] = command[1]
                        print(YELLOW + option[0] + GREEN + " ==> '" +
                              YELLOW + command[1] + GREEN + "'" + END)
                if not inOptions:
                    print(RED + "[!] Option '" + YELLOW +
                          command[0] + RED + "' not found." + END)
        else:
            print(RED + "[!] No module selected." + END)
    elif command == "set":
        print(RED + "[!] Usage: 'set " + YELLOW +
    ...
    # MODULES
        ...
        for module in allModules:
            print(YELLOW + "\t" + module[0] +
                  GREEN + " - " + BLUE + module[1] + END)
        print()
    ...
    # DEBUG
        ...
        print(RED + "[!] Unknown command: '" + YELLOW + command + RED +
              "'. Type '" + YELLOW + "help" + RED + "' for all available commands." + END)
...
header = """
                  _   _____                                 
                 | | /  ___|                                
 _ __   ___  _ __| |_\ `--.  ___ __ _ _ __  _ __   ___ _ __ 
| '_ \ / _ \| '__| __|`--. \/ __/ _` | '_ \| '_ \ / _ \ '__|
| |_) | (_) | |  | |_/\__/ / (_| (_| | | | | | | |  __/ |   
| .__/ \___/|_|   \__\____/ \___\__,_|_| |_|_| |_|\___|_|   
| |                                                         
|_|                                                         
"""
...
while True:
    if inModule:
        inputHeader = BLUE + "portScanner" + RED + \
            "/" + currentModule + BLUE + " $> " + END
    else:
        inputHeader = BLUE + "portScanner $> " + END
