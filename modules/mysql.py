...
def checkSQL(host, port):
    ...
    if loginFail:
        try:
            con = pymysql.connect(host=host, user='root',
                                  passwd='', connect_timeout=sqlTimeout)
        except:
            return ["login-error"]
...
def coreOptions():
    options = [["network", "IP range to scan", ""], ["port", "Port to scan.", "3306"],
               ...
               ["sql-timeout",
                   "Timeout (in sec) for the database connection.", "3"],
...
def core(moduleOptions):
    print(
        "\n" + GREEN + "MYSQL module. Scanning subnet '" + YELLOW + moduleOptions[0][2] + GREEN + "'...\n")
    ...
    fileName = script_path + "logs/log-mysql-portScanner-" + i + ".log"
    ...
    print("\n\n" + GREEN + "[I] MYSQL module done. Results saved to '" +
