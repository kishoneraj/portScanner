...
def getHTTP(address, port):
    ...
    try:
        r = requests.get("http://" + address,
                         timeout=float(titleTimeout), allow_redirects=True)
    except:
        return False
        ...
        for port in ports:
            ...
            if isUp != "FAIL":
                if isUp:
                    ...
                    print1(GREEN + "[+] Port " + str(port) +
                           " is open on '" + stringIP + "'" + END)

                    if port == 80:
                        http = getHTTP(stringIP, 80)
                        if not http:
                            print1(
                                YELLOW + "[!] Failed to get the HTTP response of '" + stringIP + "'" + END)
                            title = "NO-TITLE"
                            code = "NO-CODE"
                        else:
                            title = str(http[0])
                            ...
                            if code is not None:
                                print1(
                                    GREEN + "[+] Response code of '" + stringIP + "': '" + code + "'" + END)
                            else:
                                print1(
                                    YELLOW + "[!] Failed to get the response code of '" + stringIP + "'" + YELLOW)
                                code = "NO-CODE"
                            if title is not None:
                                title = title.replace("\n", "")
                                try:
                                    print1(
                                        GREEN + "[+] Title of '" + stringIP + "': '" + title + "'" + END)
                                except:
                                    print1(
                                        YELLOW + "[!] Failed to print title of '" + stringIP + "'" + END)
                                    title = "NO-TITLE"
                            else:
                                print1(
                                    YELLOW + "[!] Failed to get title of '" + stringIP + "'" + YELLOW)
                                title = "NO-TITLE"
                    ...
                    print1(RED + "[-] Port " + str(port) +
                           " is closed on '" + stringIP + "'" + END)
            else:
                print1(RED + "[!] Failed connecting to '" +
                       stringIP + "'" + END)
    done = done + 1
...
def core(moduleOptions):
    print(
        "\n" + GREEN + "PRINTER module. Scanning subnet '" + YELLOW + moduleOptions[0][2] + GREEN + "'...\n")
    ...
    fileName = script_path + "logs/log-printer-portScanner-" + i + ".log"
    ...
    print("\n\n" + GREEN + "[I] PRINTER module done. Results saved to '" +
