            ...
            if isUp != "FAIL":
                if isUp:
                    ...
                    print1(GREEN + "[+] Port " + str(port) +
                           " is open on '" + stringIP + "' - " + possibleGame + END)
                    logLine = stringIP + " - " + \
                        str(port) + " OPEN - " + possibleGame + "\n"
...
def core(moduleOptions):
    print(
        "\n" + GREEN + "GameServer module. Scanning subnet '" + YELLOW + moduleOptions[0][2] + GREEN + "'...\n")
    ...
    fileName = script_path + "logs/log-gameserver-portScanner-" + i + ".log"
    ...
    print("\n\n" + GREEN + "[I] GameServer module done. Results saved to '" +
